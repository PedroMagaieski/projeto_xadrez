...
public class Tabuleiro {
	...
	public Tabuleiro(ControlaTempo controleTempo) {
		...
		//inicializa peças na posição inicial relativa das peças
	...
	public void selecionaPeca(Peca peca) {
		if(peca.isSelecionada()){//se peça já estiver selecionada, desmarca a peça
	...
	public void movePeca(Peca peca, int novaLinha,int novaColuna) {
		...
		if(peca.validaMovimento(novaLinha, novaColuna)) {
			...
			if(peca instanceof Peao) {//checa se peca selecionada é um peao e após mover muda o primeiro movimento para falso
				Peao peao = (Peao) peca;
	...
	public void realizaJogada(int linha, int coluna) {
		...
		if(this.pecaSelecioada == null) {
			...
			if(this.pecaSelecioada == peca) {
				...
				if(peca != null && !peca.getCor().equals(this.pecaSelecioada.getCor())) { // comer peca adversaria, peca não nula e com a cor oposta
